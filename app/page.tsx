...
import { Download, Flame, Loader, Mic, Pause, Play, Plus, X } from "lucide-react"
...
export default function Home() {
  ...
  // 1. Create refs for audio and scroll area
  const audioRef = useRef<HTMLAudioElement>(null)
  const scrollAreaRef = useRef<HTMLDivElement>(null)

  // 2. Function to validate URL
  ...
  // 4. Function to remove a URL
  const removeUrl = (urlToRemove: string) => {
    setUrls(urls.filter(url => url !== urlToRemove))
  }

  // 5. Function to fetch news and generate podcast
  ...
  // 6. Function to toggle play/pause of audio
  ...
  // 7. Function to download the generated audio
  const downloadAudio = () => {
    if (audioSrc) {
  ...
  // 8. Use effect hook to set up audio event listeners
  useEffect(() => {
    if (audioRef.current) {
  ...
  // 9. Define gradient animation for loading state
            ...
            <CardContent className="p-6 flex flex-col lg:flex-row gap-6 h-[calc(100vh-12rem)]">
              ...
              >
                ...
                <Button
                  onClick={fetchNews}
                  disabled={isLoading}
                  className="w-full bg-gradient-to-r from-orange-500 to-black text-white font-light cursor-pointer"
                >
                  {isLoading ? (
                    <>
              ...
              {/* 14. Podcast content and audio player section */}
              <AnimatePresence>
                {isExpanded && (
                  <motion.div
